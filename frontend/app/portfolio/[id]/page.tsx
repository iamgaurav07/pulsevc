  ...
  return (
    <div
      style={{
        ...
        height,
        background:
          "linear-gradient(90deg, var(--bg-card) 25%, #1f2235 50%, var(--bg-card) 75%)",
...
export default function PortfolioPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "forecast" | "anomaly" | "chat"
  >("overview");
  ...
  const handleTabChange = (tab: typeof activeTab) => {
    setActiveTab(tab);
    if (tab === "anomaly" && !anomalyData && !analysisLoading)
      runAnomalyAnalysis();
  ...
  const filteredCompanies = (portfolio?.companies ?? [])
    .filter(
      (c) =>
    ...
    .sort((a, b) => {
      ...
      let valB = 0;
      if (sortBy === "revenue") {
        valA = a.latest_revenue ?? 0;
        valB = b.latest_revenue ?? 0;
      }
      if (sortBy === "runway") {
        valA = a.latest_runway ?? 0;
        valB = b.latest_runway ?? 0;
      }
      if (sortBy === "name")
        return sortDir === "asc"
  ...
  // forecast chart data
  const selectedForecast = forecastData?.companies?.find(
          ...
          <div>
              ...
              {[
                {
                  ...
                  value: portfolio?.companies?.length ?? 0,
                  icon: "🏢",
                  format: false,
                },
                {
                  ...
                  value: formatCurrency(
                    (portfolio?.companies?.reduce(
                      (s, c) => s + (c.latest_revenue ?? 0),
                      0,
                    ) ?? 0) / (portfolio?.companies?.length || 1),
                  ),
                  icon: "💰",
                  format: true,
                },
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <option value="runway">Sort by Runway</option>
                ...
                >
                  {filteredCompanies.length} of {portfolio?.companies?.length}{" "}
                  companies
                </span>
              </div>
              ...
              {(() => {
                const sectors = [
                  ...new Set(
                    portfolio?.companies?.map((c) => c.sector).filter(Boolean),
                  ),
                ];
                  ...
                  >
                    <button
                      ...
                      style={{
                        ...
                        cursor: "pointer",
                        borderColor:
                          search === "" ? "var(--accent)" : "var(--border)",
                        background:
                          search === ""
                            ? "var(--accent-light)"
                            : "var(--bg-card)",
                        color:
                          search === "" ? "#818cf8" : "var(--text-tertiary)",
                      }}
                    >
                      All
                    </button>
              ...
              {paginatedCompanies.map((company, i) => (
                ...
                >
                  ...
                  <div
                    style={{ display: "flex", gap: "32px", textAlign: "right" }}
                  >
                    <div>
                    ...
                    <div>
                      <p
                        style={{
                          ...
                          fontWeight: "600",
                          color:
                            (company.latest_runway ?? 0) < 6
                              ? "#f87171"
                              : (company.latest_runway ?? 0) < 12
                                ? "#fbbf24"
                                : "#34d399",
                        }}
                      >
                        {company.latest_runway != null
                          ? `${company.latest_runway} mo`
                          : "N/A"}
                      </p>
                      <p
                        style={{
            ...
            {analysisLoading ? (
              <div>
                ...
                >
                  ...
                  >
                    Analyzing {portfolio?.companies?.length} companies — may
                    take 15–30 seconds
                  </p>
                  <div
                    style={{
                    ...
                    >
                      ...
                      <div
                        style={{
                          ...
                          borderRadius: "20px",
                          background:
                            selectedForecast.forecast.revenue.trend === "up"
                              ? "var(--success-light)"
                              : "var(--danger-light)",
                          color:
                            selectedForecast.forecast.revenue.trend === "up"
                      ...
                      >
                        {selectedForecast.forecast.revenue.trend === "up"
                          ? "↑"
                          : "↓"}{" "}
                        {selectedForecast.forecast.revenue.growth_forecast}%
                        forecast
                      </div>
                    </div>
                ...
                >
                  {forecastData.companies.map((c) => (
                    <div
                      ...
                      style={{
                        background: "var(--bg-card)",
                        border: `1px solid ${selectedCompany === c.company_id ? "var(--accent)" : "var(--border)"}`,
                        borderRadius: "var(--radius)",
                        padding: "14px",
                        cursor: "pointer",
                        transition: "all 0.15s",
                    ...
                    >
                      ...
                      {c.forecast?.revenue ? (
                        ...
                        >
                          ...
                          >
                            {c.forecast.revenue.trend === "up" ? "↑" : "↓"}{" "}
                            {c.forecast.revenue.growth_forecast}%
                          </span>
                          <span
                            style={{
                              fontSize: "11px",
            ...
            {analysisLoading ? (
              <div>
                ...
                >
                  ...
                  >
                    Scanning {portfolio?.companies?.length} companies for
                    unusual patterns
                  </p>
                  <div
                    style={{
              ...
              <div>
                ...
                {/* Critical alerts */}
                {anomalyData.alerts?.filter((a) => a.severity === "critical")
                  .length > 0 && (
                  <div
                    style={{
                ...
                >
                  {anomalyData.companies.map((company) => {
                    const s =
                      ...
                      statusColors.insufficient_data;
                    return (
                      <div
                          ...
                          {[
                            {
                              label: "Revenue",
                              value: formatCurrency(
                                company.latest_metrics?.revenue,
                              ),
                            },
                            {
                              label: "Growth",
                              value:
                                company.latest_metrics?.growth_rate != null
                                  ? `${company.latest_metrics.growth_rate}%`
                                  : "N/A",
                            },
                            {
                              label: "Burn",
                              value: formatCurrency(
                                company.latest_metrics?.burn_rate,
                              ),
                            },
                            {
                              label: "Runway",
                              value:
                                company.latest_metrics?.runway_months != null
                                  ? `${company.latest_metrics.runway_months} mo`
                                  : "N/A",
                            },
                          ].map((m) => (
                            <div
